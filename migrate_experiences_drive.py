...
Görselleri Google Drive'a yükler ve URL'lerini saklar
"""
...
def migrate_experiences_with_drive():
    """SQLite'taki experiences tablosunu Google Sheets'e aktarır (Drive ile)"""
    ...
    for exp in experiences:
        ...
        if image_path:
            ...
            if os.path.exists(full_path):
                # Dosya adı oluştur
                filename = f"experience_{exp_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"

                # Drive'a yükle
                image_url = upload_image_to_drive(full_path, filename, is_path=True)

                if image_url:
                    print(f"   ✅ Görsel Drive'a yüklendi!")
                else:
...
if __name__ == "__main__":
    print("🔄 Trade Journal → Google Sheets + Drive Migration Başlıyor...")
    print("="*60)
    migrate_experiences_with_drive()
